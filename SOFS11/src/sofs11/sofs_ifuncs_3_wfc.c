/**
 *  \file sofs_ifuncs_3_wfc (implementation file for function soWriteFileCluster)
 *
...
{
  soProbe (212, "soWriteFileCluster (%"PRIu32", %"PRIu32", %p)\n", nInode, clustInd, buff);
  ...
  /** Get cluster's logical number **/
  if((error = soHandleFileCluster(nInode, clustInd, GET, &logClustNum)) != 0)
    return error;

  /** Check if cluster needs to be allocated **/
  if(logClustNum == NULL_CLUSTER)
    if((error = soHandleFileCluster(nInode, clustInd, ALLOC, &logClustNum)) != 0)
      return error;
  ...
  /** Update and write data cluster **/
  memcpy(&Cluster.info, buff, BSLPC);
  if((error = soWriteCacheCluster(phyClustNum, &Cluster)) != 0)
    return error;
