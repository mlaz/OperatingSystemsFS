...
int soAllocInode (uint32_t type, uint32_t* p_nInode)
{
  ...
  /*if((headInode[headOffset].mode & INODE_FREE) != INODE_FREE)
    return -ELIBBAD;*/
  ...
  *p_nInode = sb->ihead;  
  ...
  if(sb->ifree == 1){
    /*TODO verificar se ihead e itail são iguais*/
    sb->ihead = NULL_INODE;
    sb->itail = NULL_INODE;
  }
  else
  ...
  if(headInode[headOffset].d[0] != NULL_CLUSTER)
    memset(&headInode[headOffset], 0, sizeof(SOInode));
  ...
  headInode[headOffset].vD2.mtime = time(NULL);
  /*Update cluster references*/
  for(i = 0; i < N_DIRECT; i++)
    headInode[headOffset].d[i] = NULL_CLUSTER;
  headInode[headOffset].i1 = NULL_CLUSTER;
  headInode[headOffset].i2 = NULL_CLUSTER;
