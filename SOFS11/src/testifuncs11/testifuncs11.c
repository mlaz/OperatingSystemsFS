/**
 *  \file testifuncs11.c (implementation file)
 *
 *  \brief The SOFS11 internal testing tool.
 *
 ...
 *
 *  SINOPSIS:
 *  <P><PRE>                testifuncs11 [OPTIONS] supp-file
 ...
 *  \author António Rui Borges - October 2010 / September 2011
 */
...
static void addDirEntry (void);
static void removeDirEntry (void);
static void renameDirEntry (void);
static void checkDirectoryEmptiness (void);
static void attachDirectory (void);
static void detachDirEntry (void);
...
#endif
#ifdef IFUNCS_4
                         getDirEntryByPath,      /* 13 */
                         getDirEntryByName,      /* 14 */
                         addDirEntry,            /* 15 */
                         removeDirEntry,         /* 16 */
                         renameDirEntry,         /* 17 */
                         checkDirectoryEmptiness,/* 18 */
                         attachDirectory,        /* 19 */
                         detachDirEntry,         /* 20 */
                         initSymLink             /* 21 */
#endif
...
static void printMenu (void)
{
  printf(
               ...
               "+--------------------------------------------------------------+\n"
               "| 13 - soGetDirEntryByPath    14 - soGetDirEntryByName         |\n"
               "| 15 - soAddDirEntry          16 - soRemoveDirEntry            |\n"
               "| 17 - soRenameDirEntry       18 - soCheckDirectoryEmptiness   |\n"
               "| 19 - soAttachDirectory      20 - soDetachDirEntry            |\n"
               "+--------------------------------------------------------------+\n"
               "| 21 - soInitSymLink                                           |\n"
               "+==============================================================+\n");
}
  ...
  if (p_inode->mode & INODE_FREE)
     ...
     { fprintf (fl, "prev = ");
       ...
       if (p_inode->vD1.next == NULL_INODE)
          ...
          { ctime_r ((time_t *) &(p_inode->vD1.atime), timebuf);
            timebuf[strlen (timebuf) - 1] = '\0';
            fprintf (fl, "atime = %s, ", timebuf);
            ctime_r ((time_t *) &(p_inode->vD2.mtime), timebuf);
            timebuf[strlen (timebuf) - 1] = '\0';
            fprintf (fl, "mtime = %s\n", timebuf);
          }
  fprintf (fl, "d[] = {");
...
static void writeFileCluster (void)
{
  ...
  if ((stat = soWriteFileCluster (nInode, nClust, dc.info.data)) != 0)
     printError (stat, "soWriteFileCluster");
            ...
            fprintf(fl, "Cluster with index no. %u successfully written.\n", nClust);
          }
}
...
static void handleFileCluster (void)
{
  ...
  if (batch == 0)
     { printf("Operation to be performed:\n");
       printf("    0 - get the physical number of the data cluster\n");
       printf("    1 - allocate a free data cluster and associate it to the inode which describes the file\n");
  ...
  if (stat != 0)
     printError (stat, "soHandleFileCluster");
            ...
            { case 0:  if (npClust == NULL_CLUSTER)
                          fprintf(fl, "Logical cluster no. is (nil).\n");
                          else fprintf(fl, "Logical cluster no. is %u.\n", npClust);
                       break;
              case 1:  fprintf(fl, "Logical cluster no. %u successfully allocated.\n", npClust);
                       break;
              case 2:  fprintf(fl, "Cluster whose index to the list of direct references is %u successfully freed.\n",
                               nClust);
                       break;
...
static void addDirEntry (void)
{
  ...
  stat = soAddDirEntry (nInodeDir, name, nInodeEnt);
  if (stat != 0)
     printError (stat, "soAddDirEntry");
     else { if (fl == stdout) fprintf (fl, "\e[07;32m==>\e[0m ");
            fprintf(fl, "The entry whose name is %s was successfully added to the parent directory.\n", name);
          }
}
...
static void removeDirEntry (void)
{
  ...
  stat = soRemoveDirEntry (nInodeDir, name);
  if (stat != 0)
     printError (stat, "soRemoveDirEntry");
     else { if (fl == stdout) fprintf (fl, "\e[07;32m==>\e[0m ");
            fprintf(fl, "The entry whose name is %s was successfully removed from the parent directory.\n", name);
          }
}
...
static void attachDirectory (void)
{
  ...
  int valInt;                                    /* integer value */
  char name[MAX_NAME+1];                         /* entry name */
  uint32_t nInodeDir;                            /* number of the base directory inode */
  uint32_t nInodeEnt;                            /* number of the subsidiary directory inode */
  int stat;                                      /* status of operation */

  if ((batch != 0) && (fl != stdout)) fprintf (fl, "Add a directory entry\n");
  if (batch == 0) printf("Inode number of the base directory: ");
  do
  { t = scanf ("%d", &valInt);
  } while (t != 1);
  nInodeDir = (uint32_t) valInt;
  ...
  nInodeEnt = (uint32_t) valInt;
  if (batch == 0) printf("Name of the the subsidiary directory: ");
  do
  { t = scanf ("%59s", name);
    scanf ("%*[^\n]");
    scanf ("%*c");
  } while (t != 1);
  stat = soAttachDirectory (nInodeDir, name, nInodeEnt);
  if (stat != 0)
     printError (stat, "soAttachDirectory");
     else { if (fl == stdout) fprintf (fl, "\e[07;32m==>\e[0m ");
            fprintf(fl, "The entry whose name is %s was successfully attached to the base directory.\n", name);
          }
}

/*
 * detach a directory entry
 */
...
static void detachDirEntry (void)
{
  ...
  if ((batch != 0) && (fl != stdout)) fprintf (fl, "Detach a directory entry\n");
  if (batch == 0) printf("Inode number of the directory: ");
  do
  { t = scanf ("%d", &valInt);
  } while (t != 1);
  ...
  stat = soDetachDirEntry (nInodeDir, name);
  if (stat != 0)
     printError (stat, "soDetachDirEntry");
     else { if (fl == stdout) fprintf (fl, "\e[07;32m==>\e[0m ");
            fprintf(fl, "The entry whose name is %s was successfully detached from the parent directory.\n", name);
          }
}
