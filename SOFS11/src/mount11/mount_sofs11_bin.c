/**
 *  \file mount_sofs11.c (implementation file)
 *
 *  \brief The SOFS11 mounting tool.
 *
 *  It provides a simple method to integrate the SOFS11 file system into Linux.
 * *
 *  SINOPSIS:
 *  <P><PRE>                mount_sofs11 [OPTIONS] supp-file mount-point
...
int main(int argc, char *argv[])
{
  ...
  char *fuse_argv[] = {argv[0], argv[optind+1], "-o", "nonempty", "-d"};
  int fuse_argc = (debug_mode) ? 5 : 4;
...
static void *sofs_mount (struct fuse_conn_info* fci)
{
  soColorProbe (11, "07;31", "sofs_mount_bin ()\n");
...
{
  soColorProbe (12, "07;31", "sofs_unmount_bin (\"%s\")\n", (char *) path);
 ...
 *
 *  Equivalent to the system call stat (man 2 stat).
 *  The 'st_dev' and 'st_blksize' fields are ignored. The 'st_ino' field is ignored, except if the 'use_ino' mount
 *  option is given.
 *
...
static int sofs_getattr (const char *ePath, struct stat *st)
{
  soColorProbe (13, "07;31", "sofs_getattr_bin (\"%s\", %p)\n", ePath, st);
...
{
  soColorProbe (14, "07;31", "sofs_access_bin (\"%s\", %x)\n", ePath, opRequested);
 ...
 *
 *  Equivalent to system call mknod (man 2 mknod).
 *
...
static int sofs_mknod (const char *ePath, mode_t mode, dev_t rdev)
{
  soColorProbe (15, "07;31", "sofs_mknod_bin (\"%s\", %x, %x)\n", ePath, (uint32_t) mode, (uint32_t) rdev);
 ...
 *
 *  Equivalent to system call mkdir (man 2 mkdir).
 *
 *  \param ePath path to the file
 *  \param mode type and permissions to be set
 *
...
static int sofs_mkdir (const char *ePath, mode_t mode)
{
  soColorProbe (16, "07;31", "sofs_mkdir_bin (\"%s\", %x)\n", ePath, (uint32_t) mode);
 ...
 *
 *  Equivalent to system call unlink (man 2 unlink).
 *
 *  \param ePath path to the file
 *
 *  \return 0, on success, and a negative value, on error
...
static int sofs_unlink (const char *ePath)
{
  soColorProbe (17, "07;31", "sofs_unlink_bin (\"%s\")\n", ePath);
 ...
 *
 *  Equivalent to system call rmdir (man 2 rmdir).
 *
 *  \param ePath path to the file
 *
 *  \return 0, on success, and a negative value, on error
...
static int sofs_rmdir (const char *ePath)
{
  soColorProbe (18, "07;31", "sofs_rmdir_bin (\"%s\")\n", ePath);
 ...
 *  \brief Rename a file.
 *
 *  Equivalent to function (not system call) rename (man 2 rename).
 *
...
static int sofs_rename (const char *oldPath, const char *newPath)
{
  soColorProbe (19, "07;31", "sofs_rename_bin (\"%s\", \"%s\")\n", oldPath, newPath);
 ...
 *
 *  Equivalent to system call link (man 2 link).
 *
 *  \param oldPath path to an existing file
...
static int sofs_link (const char *oldPath, const char *newPath)
{
  soColorProbe (20, "07;31", "sofs_link_bin (\"%s\", \"%s\")\n", oldPath, newPath);
 ...
 *
 *  Equivalent to system call chmod (man 2 chmod).
 *
 *  \param ePath path to the file
...
static int sofs_chmod (const char *ePath, mode_t mode)
{
  soColorProbe (21, "07;31", "sofs_chmod_bin (\"%s\", 0%o)\n", ePath, (uint32_t) mode);
 ...
 *
 *  Equivalent to system call chown (man 2 chown).
 *
 *  \param ePath path to the file
...
static int sofs_chown (const char *ePath, uid_t owner, gid_t group)
{
  soColorProbe (22, "07;31", "sofs_chown_bin (\"%s\", %"PRIu32", %"PRIu32")\n", ePath, (uint32_t) owner, (uint32_t) group);
 ...
 *
 *  Equivalent to system call truncate (man 2 truncate).
 *
 *  \param ePath path to the file
...
static int sofs_truncate (const char *ePath, off_t length)
{
  soColorProbe (23, "07;31", "sofs_truncate_bin (\"%s\", %u)\n", ePath, (uint32_t) length);
 ...
 *
 *  Equivalent to system call utime (man 2 utime).
 *
 *  \param ePath path to the file
...
static int sofs_utime (const char *ePath, struct utimbuf *times)
{
  soColorProbe (24, "07;31", "sofs_utime_bin (\"%s\", %p)\n", ePath, times);
...
static int sofs_statfs (const char *ePath, struct statvfs *st)
{
  soColorProbe (25, "07;31", "sofs_statfs_bin (\"%s\", %p)\n", ePath, st);
...
static int sofs_open (const char *ePath, struct fuse_file_info *fi)
{
  soColorProbe (26, "07;31", "sofs_open_bin (\"%s\", %p)\n", ePath, fi);
...
{
  soColorProbe (27, "07;31", "sofs_read_bin (\"%s\", %p, %"PRIu32", %"PRId32", %p)\n", ePath, buff, (uint32_t) count,
                (int32_t) pos, fi);
...
{
  soColorProbe (28, "07;31", "sofs_write_bin (\"%s\", %p, %"PRIu32", %"PRId32", %p)\n", ePath, buff, (uint32_t) count,
                (int32_t) pos, fi);
...
static int sofs_flush (const char *ePath, struct fuse_file_info *fi)
{
  soColorProbe(29, "07;31", "sofs_flush_bin (\"%s\", %p)\n", ePath, fi);
...
static int sofs_release (const char *ePath, struct fuse_file_info *fi)
{
  soColorProbe (30, "07;31", "sofs_release_bin (\"%s\", %p)\n", ePath, fi);
...
{
  soColorProbe(31, "07;31", "sofs_fsync_bin (\"%s\", %d, %p)\n", ePath, isdatasync, fi);
...
static int sofs_opendir (const char *ePath, struct fuse_file_info *fi)
{
  soColorProbe (32, "07;31", "sofs_opendir_bin (\"%s\", %p)\n", ePath, fi);
...
static int sofs_readdir (const char *ePath, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fi)
{
  soColorProbe (33, "07;31", "sofs_readdir_bin (\"%s\", %p, %p, %"PRId32", %p)\n", ePath, buf, filler,
                (int32_t)offset, fi);
...
static int sofs_releasedir (const char *ePath, struct fuse_file_info *fi)
{
  soColorProbe (34, "07;31", "sofs_releasedir_bin (\"%s\", %p)\n", ePath, fi);
...
{
  soColorProbe (35, "07;31", "sofs_fsyncdir_bin (\"%s\", %d, %p)\n", ePath, isdatasync, fi);
 ...
 *
 *  Equivalent to system call symlink (man 2 symlink).
 *
...
static int sofs_symlink (const char *effPath, const char *ePath)
{
  soColorProbe (36, "07;31", "sofs_symlink_bin (\"%s\", \"%s\")\n", effPath, ePath);
 ...
 *
 *  Equivalent to system call readlink (man 2 readlink).
 *
 *  The buffer should be filled with a null terminated string. The buffer size argument includes the space for the
 *  terminating null character. If the linkname is too long to fit in the buffer, it should be truncated. The return
...
static int sofs_readlink (const char *ePath, char *buf, size_t size)
{
  soColorProbe (37, "07;31", "sofs_readlink_bin (\"%s\", %p, %"PRIu32")\n", ePath, buf, (uint32_t) size);
...
static int sofs_setxattr (const char *ePath, const char *name, const char *value, size_t size, int flags)
{
  soColorProbe (38, "07;31", "sofs_setxattr_bin (\"%s\", \"%s\", %p, %"PRIu32", %d)\n", ePath, name, value,
                (uint32_t) size, flags);
...
static int sofs_getxattr (const char *ePath, const char *name, char *value, size_t size)
{
  soColorProbe (39, "07;31", "sofs_getxattr_bin (\"%s\", \"%s\", %p, %"PRIu32")\n", ePath, name, value, (uint32_t) size);
...
static int sofs_listxattr (const char *ePath, char *list, size_t size)
{
  soColorProbe (40, "07;31", "sofs_listxattr_bin (\"%s\", \"%s\", %"PRIu32")\n", ePath, list, (uint32_t) size);
...
{
  soColorProbe (41, "07;31", "sofs_removexattr_bin (\"%s\", \"%s\")\n", ePath, name);
...
static int sofs_getdir (const char *ePath, fuse_dirh_t handle, fuse_dirfil_t filler)
{
  soColorProbe (42, "07;31", "sofs_getdir_bin (\"%s\", ...)\n", ePath);
